...
export async function signupWithConnection(args: {
  person: {
  ...
  const { person, company, provider } = args
  const session = await prisma.session.create({
    data: {
      expirationDate: getSessionExpirationDate(),
      user: {
        create: {
          ...
          connections: {
            create: { providerId: provider.id, providerName: provider.name },
          },
          image: person.imageUrl
            ? { create: await downloadFile(person.imageUrl) }
            : undefined,
          person: {
            create: {
              firstName: person.firstName,
